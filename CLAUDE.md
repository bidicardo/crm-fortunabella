...
- Дизайн вносится по `docs/26-ui-designe-plan.md`: сначала дизайн-система (Д1),
  затем экраны по референсам из `docs/design/refs/`; новые экраны собираются из её
  компонентов; референс описывает вид, а не логику. Иконки — Lucide (встроенные SVG, без пакета).
