...
- План внедрения дизайна и референсы: `docs/26-ui-designe-plan.md`, `docs/design/`
...
- Дизайн-система (Д1 выполнена): цвета, шрифт Inter, скругления, тени — токены в
  `resources/css/app.css` (светлая тема по умолчанию, тёмная — класс `.dark`);
  Blade-компоненты `resources/views/components/ui/*` (`<x-ui.button>`, `<x-ui.input>`,
  `<x-ui.table>`, `<x-ui.field>` и т. д.); иконки — `<x-icon name="...">` (Lucide,
  SVG встроены, без пакета; исходники в `docs/design/icons/`). Экраны собираются
  только из компонентов и токенов, без цветов Tailwind по умолчанию (slate, indigo…).
  Витрина элементов — `/design-showcase` (только `APP_ENV=local`). Дальнейшие
  экраны — по `docs/26-ui-designe-plan.md` и референсам `docs/design/refs/`;
  референс описывает вид, а не логику.
- Пагинация Livewire переопределена в `resources/views/vendor/livewire/tailwind.blade.php`.
- Подводные камни вёрстки (на них уже были ошибки):
  - не передавать компоненту класс того же свойства, что у него внутри (`px-*`,
    `w-*`, `text-*`): у двух таких классов победитель в Tailwind не определён;
  - выражение Alpine (`x-on:click` и т. п.) не должно начинаться с комментария `//` —
    Alpine его не разбирает; длинный код — методом в `x-data`;
  - абсолютно позиционированный элемент (в том числе `sr-only`) внутри блока с
    прокруткой — только в родителе с `relative`, иначе на iPhone растягивает страницу;
  - выпадающие панели, которые на узком экране могут уйти за край, — `x-anchor`
    (Alpine, в комплекте Livewire);
  - полям пароля не ставить `name` (`tests/Feature/SecurityTest.php`); компоненты
    полей ищут ошибку валидации по `id`.
...
- Только создатель может приглашать и блокировать пользователей и деактивировать приглашения.
...
- Редактирование карточек (клиент, сделка, контрагент) — прямо в карточке, по одному полю щелчком по нему (`<x-ui.editable-field>`); отдельной страницы редактирования нет; создание — отдельная страница в той же компоновке, что карточка (`docs/17-ui-screens.md`).
- Стрелка «Назад» в верхней панели ведёт в родительский раздел (карточка/форма → список раздела, список и страницы вне меню → главная), а не по истории браузера (`docs/17-ui-screens.md`).
